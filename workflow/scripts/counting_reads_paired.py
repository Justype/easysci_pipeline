...
#      --sample_ids:    Text file with sample ids
#      --gtf:           GTF file with exon information only
#      --rt_barcode_tsv: RT barcode tsv (Sample	shortdt	randomN)
#      --threads:       Number of threads
#   Output:
#      <output_prefix>.count: Count file with gene expression per cell ┐ 
#      <output_prefix>_cell_id.csv: Cell id file with unmatched rate   │ for later sparse matrix creation
#      <output_prefix_base>/gene_ids.csv: Gene exon id table           ┘
##############################################################
...
def count_pair(algn_pair, genes, exons, transcript_ends, is_randomn, counter):
    ...
    if algn_pair[0].iv.chrom not in genes.chrom_vectors or algn_pair[1].iv.chrom not in genes.chrom_vectors:
        ...
        return
    global N_TOTAL, N_EXON, N_EXON_MULTIPLE, N_INTRON, N_INTRON_MULTIPLE, N_TAIL, N_OPPOSITE_EXON, N_OPPOSITE_INTRON
    N_TOTAL += 1
    ...
    # If still not found, try flanking regions
    if len(gene_ids_intersect) == 0 and len(gene_ids_union) == 0:
        ...
        if len(gene_ids_intersect) > 0 or len(gene_ids_union) > 0:
            N_TAIL += 1
    elif len(gene_ids_intersect) == 1 or len(gene_ids_union) == 1:
        N_INTRON += 1
    elif len(gene_ids_intersect) > 1 or len(gene_ids_union) > 1:
        N_INTRON_MULTIPLE += 1
    ...
    if len(gene_ids_intersect) == 0 and len(gene_ids_union) == 0:
        ...
        if len(gene_ids_intersect) > 0:
            N_OPPOSITE_EXON += 1
        elif len(gene_ids_union) > 0:
            N_OPPOSITE_INTRON += 1
    if len(gene_ids_intersect) == 0 and len(gene_ids_union) == 0:
        gene_ids_union, gene_ids_intersect = get_union_intersect_genes(algn_pair, genes)
        if len(gene_ids_intersect) > 0 or len(gene_ids_union) > 0:
            N_OPPOSITE_INTRON += 1
    elif len(gene_ids_intersect) == 0: 
        # Found exons on the opposite strand
        suffix = ""
...
def get_union_intersect_flanking_genes(algn_pair, htseq_array, flanking_size=1000)-> tuple[set, set]:
    """
    Get genes of flanking regions using a pair of alignments
...
def count_bam(bam_path, output_prefix, genes, exons, transcript_ends, gene_id_index, randomn_barcodes):
    ...
    sample = os.path.basename(output_prefix)
    ...
    for bundle in HTSeq.pair_SAM_alignments(alignments, bundle=True):
        ...
        if barcode == previous_barcode: # Same cell
            ...
            if len(counter) == 0:
                ...
                #                cell_index, sample, previous_barcode, gene_id_index)
                # reset
                counter = collections.Counter()
                cell_index += 1
                count_pair(algn_pair, genes, exons, transcript_ends, is_randomn, counter)
    ...
    print(f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] {sample} Done")
    print("N_TOTAL, N_EXON, N_EXON_MULTIPLE, N_INTRON, N_INTRON_MULTIPLE, N_TAIL, N_OPPOSITE")
    print(N_TOTAL, N_EXON, N_EXON_MULTIPLE, N_INTRON, N_INTRON_MULTIPLE, N_TAIL, N_OPPOSITE)

def record_results(counter, output_count_mtx, output_cell_ids, cell_index, sample, barcode, gene_id_index):
    """
    ...
    output_cell_ids.write(f"{cell_index},{sample}.{barcode},{unmatched_rate}\n")

def count_bam_parallel(input_folder, output_folder, genes, exons, transcript_ends, gene_id_index, randomn_barcodes, sample_id):
    """
    Count exon reads in BAM files for a sample id (used for parallel processing)
    """
    print(f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] {sample_id} Running...")
    bam_path = os.path.join(input_folder, f"{sample_id}.filtered.dedup.bam")
    ...
    print(f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] {sample_id} Done")
...
def main():
    ...
    parser.add_argument("-s", "--sample_ids", help="Text file with sample ids", required=True)
    ...
    # if not os.path.exists(out_gene_annotation_path):
    ...
    sample_ids = list(sample_id for sample_id in open(args.sample_ids).read().splitlines() if sample_id)
    ...
    with Pool(args.threads) as pool:
        pool.map(worker_count_bam, sample_ids)
