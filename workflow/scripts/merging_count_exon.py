...
##############################################################
# merging_count_exon.py
# Description: Merge exon count files for all i7 demultiplexed files (also merge RT primers)
#   Parameters:
#     --i7_prefix_file <path>: Text file containing i7 demultiplexed file prefixes
#     --input_folder   <path>: Input folder containing count files, _cell_ids, and exon_ids
...
#   Output:
#     cell_annotation.csv: Cell annotation matrix with metadata (UMI count, exon count, PCR batch, RT ligation barcodes, ligation barcodes, RT barcodes)
#     feature_annotation.csv: Exon annotation matrix
#     expression_matrix.mtx: Sparse expression matrix
##############################################################

# Basicly this script reads the exon count files for all i7 demux files, merges them, 
...
def merge_exon_counts(i7_prefixes, input_folder):
    """
    Merge exon count files for all i7 demultiplexed files.
    """
    exon_ids_df = pd.read_csv(
        path.join(input_folder, "gene_exon_ids.csv.gz" if IS_GZIP else "gene_exon_ids.csv"),
        index_col=0)
    ...
    for i7_prefix in i7_prefixes:
        current_count_mtx, current_cell_ids_df = read_matrix(
            path.join(input_folder, i7_prefix), exon_ids_df)
    ...
    return count_mtx, cell_ids_df, exon_ids_df

def read_matrix(file_prefix, exon_ids_df):
    """
    ...
    sparse_count_mtx = sparse.coo_matrix(
        (count_mtx["count"], (count_mtx["exon_index"], count_mtx["cell_index"])),
        shape=(exon_ids_df.shape[0], cell_ids_df.shape[0]),
        dtype=float
    )
...
def merge_rt_primers(cell_ids_df, count_mtx, rt_barcode_tsv_path):
    ...
    # Recalculate 1:unmatched_rate, 2:shortdt_umi_count, 3:exon_count
    ...
    cell_ids_double_shortdt["exon_count"] += cell_ids_double_randomn["exon_count"]
    ...
    # edit metadata
    cell_ids_df["exon_count"] = (count_mtx > 0).sum(axis=0).A[0]        # re-calculate exon_count
    ...
    cell_ids_df["sample"] = cell_ids_df["rt_barcodes_shortdt"].map(rt_dt_sample_dict) # add sample column by short-dT

    # drop unnecessary columns
    cell_ids_df = cell_ids_df.drop(columns=["rt_barcodes_new", "cell_id_new", "rt_barcodes"])
...
def main():
    ...
    parser.add_argument("-i", "--input_folder", required=True, help="Input folder containing count files, cell_ids, and exon_ids")
    ...
    parser.add_argument("-t", "--rt_barcode_tsv", required=True, help="TSV file containing RT barcodes (Sample	shortdt	randomN)")
    ...
    count_mtx, cell_ids_df, exon_ids_df = merge_exon_counts(i7_prefixes, args.input_folder)

    count_mtx = count_mtx.tocsr() # convert to csr format
    # remove empty cells and add metadata (UMI count, exon count, PCR batch, barcodes)
    ...
    exon_ids_df.to_csv(path.join(args.output_folder, "feature_annotation.csv.gz" if IS_GZIP else "feature_annotation.csv"), index=False)
