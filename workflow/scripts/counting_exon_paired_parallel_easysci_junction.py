...
# NOTE: count_exon_blocks_using_gene's multiplier is set to 0.5 (1 in junction script)
...
def count_pair_with_junctions(algn_pair, exons, exon_gene_dict, counter):
    ...
    if len(first_algn_blocks_and_genes[1]) == 1 and len(second_algn_blocks_and_genes[1]) == 1:
        first_algn_gene = next(iter(first_algn_blocks_and_genes[1]))
        second_algn_gene = next(iter(second_algn_blocks_and_genes[1]))
        if first_algn_gene == second_algn_gene:
            # Both reads have the same gene, count the blocks
            count_exon_blocks_using_gene(counter, exon_gene_dict, first_algn_gene, first_algn_blocks_and_genes[0])
            count_exon_blocks_using_gene(counter, exon_gene_dict, second_algn_gene, second_algn_blocks_and_genes[0])
            return
        else:
            counter["_ambiguous"] += 1
            return
    elif len(first_algn_blocks_and_genes[1]) == 1 and len(second_algn_blocks_and_genes[1]) > 1:
        first_algn_gene = next(iter(first_algn_blocks_and_genes[1]))
        count_exon_blocks_using_gene(counter, exon_gene_dict, first_algn_gene, first_algn_blocks_and_genes[0])
        if first_algn_gene in second_algn_blocks_and_genes[1]:
            # First read has one gene, second read has multiple genes, count the blocks
            count_exon_blocks_using_gene(counter, exon_gene_dict, first_algn_gene, second_algn_blocks_and_genes[0])
        else:
            # First read has one gene, second read has multiple genes, but not the same, count as ambiguous
            counter["_ambiguous"] += 1
        return
    elif len(first_algn_blocks_and_genes[1]) > 1 and len(second_algn_blocks_and_genes[1]) == 1:
        second_algn_gene = next(iter(second_algn_blocks_and_genes[1]))
        count_exon_blocks_using_gene(counter, exon_gene_dict, second_algn_gene, second_algn_blocks_and_genes[0])
        if second_algn_gene in first_algn_blocks_and_genes[1]:
            # Second read has one gene, first read has multiple genes, count the blocks
            count_exon_blocks_using_gene(counter, exon_gene_dict, second_algn_gene, first_algn_blocks_and_genes[0])
        else:
            # Second read has one gene, first read has multiple genes, but not the same, count as ambiguous
            counter["_ambiguous"] += 1
        return
    ...
    common_gene = next(iter(common_genes))
    count_exon_blocks_using_gene(counter, exon_gene_dict, common_gene, first_algn_blocks_and_genes[0])
    count_exon_blocks_using_gene(counter, exon_gene_dict, common_gene, second_algn_blocks_and_genes[0])
    return
...
def count_exon_blocks_using_gene(counter, exon_gene_dict, gene, exon_blocks: list[set], multiplier = 0.5):
    """
